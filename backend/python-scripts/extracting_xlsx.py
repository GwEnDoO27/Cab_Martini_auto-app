...
def extract_bill_values(file_path):
    ...
    try:
        ...
        print(
            f"Error: The text file you're trying to get formatted named '{file_path}' can't be found."
        )
        sys.exit(1)
    ...
    for line in lines:
        ...
        for code in code_map:
            if code in line:
                if (
                    code in line and code not in found_codes
                ):  # Only add if code is not in found_codes
...
def formating_csv(file_name):
    if __name__ == "__main__":
        ...
        with open(
            "./uploads/totals_values.csv", "w", newline="", encoding="utf-8"
        ) as f:
            data = [
                {
                    "JOURNAL": "ACM",
                    "DATE": bill_values["date"],
                    "GENERAL": "401000",
                    "AUXILIAIRE": "401LR",
                    "REFERENCE": bill_values["reference"],
                    "LIBELLE": "Achat MB " + bill_values["reference"],
                    "DEBIT": "",
                    "CREDIT": bill_values["net_payable"],
                },
                {
                    "JOURNAL": "ACM",
                    "DATE": bill_values["date"],
            ...
            print(
                "Your EDI text file has been formatted and saved in 'totals_values.csv'"
            )
    else:
...
def XMLtoXLSX(file_name, real_file_name):
    ...
    headers = [
        elem.tag for elem in root[0]
    ]  # Gets tags of the first element's children
    ...
    for element in root:
        ...
        ]  # Get text values of child elements
        ws.append(row_data)
